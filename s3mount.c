 ...
 */
static void *find_block(struct state *s, const char *key, int version, off_t offset, size_t *nbytes, int *cache_hit)
{
...
void do_read(struct state *s, const char *key, int version, void *ptr, size_t len, off_t offset)
{
    ...
    while (remaining > 0) {
        size_t nbytes;
        int cache_hit = 0;
        void *cached_data = find_block(s, key, version, offset, &nbytes, &cache_hit);
        int to_copy = (nbytes < remaining) ? nbytes : remaining;
        memcpy(dst, cached_data, to_copy);
        dst       += to_copy;
        remaining -= to_copy;
        offset    += to_copy;
        total_read += to_copy;
...
void* fs_init(struct fuse_conn_info *conn)
{
    ...
    struct s3log_config lcfg = {
        .logfile            = s->logfile,
        .bucket             = s->bucket,
        .object_key         = s->object_key,
        .mount_point        = NULL,   /* not available from FUSE conn info */
        .s3_host            = s->host,
        .local_mode         = s->local,
        .use_http           = s->use_http,
        .nocache            = s->nocache,
        .cache_size_blocks  = CACHE_SIZE,
        .cache_block_size_mb = CACHE_BLOCK / (1024 * 1024),
    };
    log_open(&s->log, &lcfg);
    ...
    struct timeval t0 = s->log.mount_time;
    log_operation(&s->log, S3LOG_OP_INIT, "/",
                  S3LOG_NO_VERSION, S3LOG_NO_OFFSET,
                  S3LOG_NO_SIZE, S3LOG_NO_SIZE,
                  s3log_elapsed_ms(&t0),
                  S3LOG_OK, S3LOG_CACHE_NA, NULL);
    return NULL;
}
...
int fs_getattr(const char *path, struct stat *sb)
{
    ...
    long dur = s3log_elapsed_ms(&t0);
    if (val < 0) {
        log_operation(&s->log, S3LOG_OP_GETATTR, path,
                      S3LOG_NO_VERSION, S3LOG_NO_OFFSET,
                      S3LOG_NO_SIZE, S3LOG_NO_SIZE,
                      dur, S3LOG_ERROR, S3LOG_CACHE_NA,
                      strerror(-val));
        return val;
    }
    dirent2stat(sb, &de);
    log_operation(&s->log, S3LOG_OP_GETATTR, path,
                  S3LOG_NO_VERSION, S3LOG_NO_OFFSET,
                  S3LOG_NO_SIZE, S3LOG_NO_SIZE,
                  dur, S3LOG_OK, S3LOG_CACHE_NA, NULL);
    return 0;
}
		       ...
		       off_t offset, struct fuse_file_info *fi)
{
    ...
    free(_path);
    if (val < 0) {
        log_operation(&s->log, S3LOG_OP_READDIR, path,
                      S3LOG_NO_VERSION, S3LOG_NO_OFFSET,
                      S3LOG_NO_SIZE, S3LOG_NO_SIZE,
                      s3log_elapsed_ms(&t0), S3LOG_ERROR,
    ...
    if (!S_ISDIR(de.mode)) {
        log_operation(&s->log, S3LOG_OP_READDIR, path,
                      ...
                      S3LOG_NO_SIZE, S3LOG_NO_SIZE,
                      s3log_elapsed_ms(&t0), S3LOG_ERROR,
                      S3LOG_CACHE_NA, strerror(ENOTDIR));
        return -ENOTDIR;
    }
...
int fs_readlink(const char *path, char *buf, size_t len)
{
    ...
    if (!S_ISLNK(de.mode)) {
        log_operation(&s->log, S3LOG_OP_READLINK, path,
                      ...
                      len, S3LOG_NO_SIZE,
                      s3log_elapsed_ms(&t0), S3LOG_ERROR,
                      S3LOG_CACHE_NA, strerror(EINVAL));
        return -EINVAL;
    }
...
/* destroy - called on filesystem unmount. Write log footer and close. */
void fs_destroy(void *userdata)
{
    log_close(&_state.log);
}
