...
LDFLAGS = -L "$(S3DIR)/build/lib" -Wl,-rpath,"$(S3DIR)/build/lib"
...
s3mount: s3mount.o s3log.o
 
s3log.o: s3log.c s3log.h
 
s3mount.o: s3mount.c s3log.h
...
clean:
	rm -f s3backup s3mount s3mount.o s3log.o